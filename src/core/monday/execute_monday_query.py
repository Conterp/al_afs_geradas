        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "errors" in result:
                    ...
                    if (
                        "JsonParseException" in error_text
                        or "Syntax error in JSON input" in error_text
                    ):
                        if MOSTRAR_ERROS_DE_PAYLOAD:
                            log_error(
                                f"{operation_name} falhou por payload invalido: {errors}"
                            )
                        raise ValueError(f"{operation_name} falhou por payload invalido")
            ...
            if should_retry_http_status(response.status_code):
                if attempt < MAX_RETRIES:
                    ...
                    log_warn(
                        f"{operation_name} recebeu HTTP {response.status_code}. "
            ...
            log_error(
                f"{operation_name} falhou com HTTP {response.status_code}: "
                f"{response.text[:500]}"
            )
            response.raise_for_status()
...
if __name__ == "__main__":
    log_info("Modulo execute_monday_query carregado com sucesso.")
    log_info(f"API_URL configurada: {API_URL}")
    log_info(f"MAX_RETRIES configurado: {MAX_RETRIES}")
    log_info("Nenhuma query foi executada neste teste local.")
